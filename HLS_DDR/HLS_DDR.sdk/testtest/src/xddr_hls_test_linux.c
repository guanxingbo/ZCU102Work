...
typedef struct {
    ...
    XDdr_hls_test_uio_map maps[ MAX_UIO_MAPS ];
} XDdr_hls_test_uio_info;

/***************** Variable Definitions **************************************/
static XDdr_hls_test_uio_info uio_info;
...
static int uio_info_read_name(XDdr_hls_test_uio_info* info) {
    char file[ MAX_UIO_PATH_SIZE ];
    sprintf(file, "/sys/class/uio/uio%d/name", info->uio_num);
    return line_from_file(file, info->name);
}

static int uio_info_read_version(XDdr_hls_test_uio_info* info) {
    char file[ MAX_UIO_PATH_SIZE ];
    sprintf(file, "/sys/class/uio/uio%d/version", info->uio_num);
    return line_from_file(file, info->version);
}

static int uio_info_read_map_addr(XDdr_hls_test_uio_info* info, int n) {
    int ret;
    char file[ MAX_UIO_PATH_SIZE ];
...
static int uio_info_read_map_size(XDdr_hls_test_uio_info* info, int n) {
    int ret;
    char file[ MAX_UIO_PATH_SIZE ];
...
int XDdr_hls_test_Initialize(XDdr_hls_test *InstancePtr, const char* InstanceName) {
	XDdr_hls_test_uio_info *InfoPtr = &uio_info;
